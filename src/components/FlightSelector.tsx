...
const FlightSelctor = () => {
  const flightPrices = new Map([
    ...
    [FlightType.THERMAL, 120],
    [FlightType.SUNSET, 150]
  ]);

  const [selectedFlight, setSelectedFlight] = useState<{type: FlightType, price: number} | null>(null);
    ...
    <section>
      ...
      <div className="flex gap-6">
        <button
          ...
          className={`relative h-20 flex-1 rounded-lg border-2 ${selectedFlight?.type === FlightType.GLIDE ? 'border-orange-500' : 'border-slate-400'} overflow-hidden hover:shadow-lg transition-shadow`}
        >
          <p
        ...
        <button
          ...
          className={`relative h-20 flex-1 rounded-lg border-2 ${selectedFlight?.type === FlightType.THERMAL ? 'border-orange-500' : 'border-slate-400'} overflow-hidden transition-shadow hover:shadow-lg hover:bg-slate-100 tracking-wide`}
        >
          <p
        ...
        <button
          ...
          className={`bg-amber-100 relative h-20 flex-1 rounded-lg border-2 ${selectedFlight?.type=== FlightType.SUNSET ? 'border-orange-500' : 'border-slate-400'} overflow-hidden transition-shadow hover:shadow-lg hover:bg-[#fdeeb4] tracking-wide`}
          >
          <p
            className="absolute font-display font-medium top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
          >
